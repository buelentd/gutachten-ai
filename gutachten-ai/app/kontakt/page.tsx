...
export default function Kontakt() {
  const [form, setForm] = useState({
    vorname: "", nachname: "", email: "", betreff: "Produktanfrage", nachricht: "", datenschutz: false,
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setStatus("success");
        setForm({ vorname: "", nachname: "", email: "", betreff: "Produktanfrage", nachricht: "", datenschutz: false });
      } else {
        setStatus("error");
      }
    } catch {
            ...
            {status === "success" ? (
              ...
              <form className="space-y-8" onSubmit={handleSubmit}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-2">
                    ...
                    <input name="vorname" value={form.vorname} onChange={handleChange}
                      className="w-full bg-surface border-[0.5px] border-technical-line rounded-lg px-4 py-3 text-on-surface focus:outline-none focus:border-[#E8631A] transition-colors placeholder:text-outline-variant"
                  ...
                  <div className="space-y-2">
                    ...
                    <input name="nachname" value={form.nachname} onChange={handleChange}
                      className="w-full bg-surface border-[0.5px] border-technical-line rounded-lg px-4 py-3 text-on-surface focus:outline-none focus:border-[#E8631A] transition-colors placeholder:text-outline-variant"
                      placeholder="Mustermann" type="text"/>
                  </div>
                </div>
                <div className="space-y-2">
                  ...
                  <input name="email" value={form.email} onChange={handleChange} required
                    className="w-full bg-surface border-[0.5px] border-technical-line rounded-lg px-4 py-3 text-on-surface focus:outline-none focus:border-[#E8631A] transition-colors placeholder:text-outline-variant"
                    placeholder="max@beispiel.de" type="email"/>
                </div>
                <div className="space-y-2">
                  <label className="text-xs font-medium text-on-surface-variant uppercase tracking-wider">Betreff</label>
                  <select name="betreff" value={form.betreff} onChange={handleChange}
                    className="w-full bg-surface border-[0.5px] border-technical-line rounded-lg px-4 py-3 text-on-surface focus:outline-none focus:border-[#E8631A] transition-colors">
                ...
                <div className="space-y-2">
                  ...
                  <textarea name="nachricht" value={form.nachricht} onChange={handleChange} required
                    className="w-full bg-surface border-[0.5px] border-technical-line rounded-lg px-4 py-3 text-on-surface focus:outline-none focus:border-[#E8631A] transition-colors resize-none placeholder:text-outline-variant"
                    placeholder="Wie können wir Ihnen helfen?" rows={5}/>
                </div>
                <div className="flex items-start gap-3">
                  <input name="datenschutz" checked={form.datenschutz} onChange={handleChange}
                    className="mt-1" id="privacy" type="checkbox" required/>
                ...
                <button
                  type="submit"
                  disabled={status === "loading" || !form.datenschutz}
        ...
        <div className="md:col-span-5 space-y-12">
          <div className="space-y-6">
            <div className="h-16 w-16 rounded-full overflow-hidden grayscale contrast-125 border-[0.5px] border-[#E8631A]">
              ...
              <img className="w-full h-full object-cover" alt="Support"
                src="https://lh3.googleusercontent.com/aida-public/AB6AXuC6IwzJ71Te-aDcwhFw-Er9ELQqUbyVk2xwaVsc-0PoyB30rhwznDRTA1tgMkIKpPNX7qC6o20hpLtrWDFJo9dJWNbBrKNBGCjh7ebDlLzWupo_RkHeTsX9yF_Q3V2k3kU1DVKzvuAZd2eqbaaL_63nDtUN-zy9etPvJXAYlfzAoyMOawzCP28NILxPbejZxiT5VuzOwbVcG_mP0PHxzG4kH8s22ewjhTfgRxza0C7nFvEjfmlapqs_mo_IuzYcemwWx_iuB0UMytJX"/>
            </div>
            <div>
