    ...
    <main className="pt-32 pb-24 max-w-7xl mx-auto px-6">
      ...
      {featured && (
        <Link href={`/blog/${featured.slug}`} className="block group mb-24">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center p-8 rounded-2xl border-[0.5px] border-technical-line hover:border-[#C54F0E]/40 transition-colors bg-surface-container-low">
            <div className="md:col-span-8">
              <div className="flex items-center gap-4 mb-4">
                {featured.category && <span className="text-[#C54F0E] text-xs font-medium tracking-widest uppercase">{featured.category}</span>}
                <span className="text-on-surface-variant/40 text-xs">•</span>
                <time className="text-on-surface-variant text-xs">{formatDate(featured.publishedAt)}</time>
              </div>
              <h2 className="text-3xl font-medium text-on-surface mb-4 group-hover:text-[#C54F0E] transition-colors">{featured.title}</h2>
              <p className="text-on-surface-variant leading-relaxed text-lg mb-6 max-w-2xl">{featured.excerpt}</p>
              <span className="inline-flex items-center gap-2 text-[#C54F0E] font-medium group-hover:gap-4 transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {rest.map((post: { slug: string; title: string; excerpt: string; category: string; publishedAt: string }) => (
          <Link key={post.slug} href={`/blog/${post.slug}`} className="group block">
            <article className="h-full p-6 rounded-xl border-[0.5px] border-technical-line hover:border-[#C54F0E]/40 transition-colors bg-surface-container-low">
              {post.category && <span className="text-[#C54F0E] text-xs font-medium tracking-widest uppercase mb-4 block">{post.category}</span>}
              <time className="text-on-surface-variant text-xs mb-3 block">{formatDate(post.publishedAt)}</time>
              <h3 className="text-lg font-medium text-on-surface mb-3 group-hover:text-[#C54F0E] transition-colors leading-snug">{post.title}</h3>
              <p className="text-on-surface-variant text-sm leading-relaxed mb-4 line-clamp-3">{post.excerpt}</p>
              <span className="text-[#C54F0E] text-sm font-medium inline-flex items-center gap-1 group-hover:gap-3 transition-all">
